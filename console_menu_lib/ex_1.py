...
def main():
    ...
    function_item = FunctionItem("Fun item", Screen().input,
                                 kwargs={"prompt": "Enter an input: "})

    # Create a menu item that calls a system command, based on OS type
    if sys.platform.startswith('win'):
        command_item = CommandItem("Command", 'cmd /c \"echo this is a '
                                              'shell. Press enter to '
                                              'continue." && set /p=\"')
    else:
        command_item = CommandItem("Command", 'sh -c \'echo "this is a '
                                              'shell. Press enter to '
                                              'continue."; read\'')

    # Create a submenu using a Selection Menu, which takes a
    # list of strings to create the menu items.
    ...
    # Create a second submenu, but this time use a
    # standard ConsoleMenu instance
