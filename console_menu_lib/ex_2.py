...
def main():
    ...
    menu = ConsoleMenu("Root Menu",
                       "This is the Root Menu Subtitle",
                       formatter=menu_format)
    item1 = MenuItem("Item 1", menu)

    # Create a menu item that calls a function
    function_item = FunctionItem("Fun item",
                                 Screen().input,
                                 kwargs={"prompt": "Enter an input: "})

    # Create a menu item that calls a system command, based on OS type
    if sys.platform.startswith('win'):
        command_item = CommandItem("Command",
                                   'cmd /c \"echo this is a shell. '
                                   'Press enter to continue." && set /p=\"')
    else:
        command_item = CommandItem("Command",
                                   'sh -c \'echo "this is a shell. '
                                   'Press enter to continue."; read\'')

    # Create a submenu using a Selection Menu, which takes a list of
    # strings to create the menu items. This submenu is passed the
    # same formatter object, to keep its formatting consistent.
    submenu = SelectionMenu(["item1", "item2", "item3"],
                            title="Selection Menu",
                            subtitle="These menu items return to the "
                                     "previous menu",
                            formatter=menu_format)
    ...
    submenu_formatter = MenuFormatBuilder().\
        set_border_style_type(MenuBorderStyleType.ASCII_BORDER)

    # Create a second submenu, but this time use a standard ConsoleMenu
    # instance, and use the submenu_formatter.
    submenu_2 = ConsoleMenu("Another Submenu Title",
                            "Submenu subtitle. Notice this menu is ASCII.",
    ...
    submenu_3 = ConsoleMenu("Third Submenu",
                            "This Time with Double-Line Borders.",
                            prologue_text="This is my prologue. I am "
                                          "currently showing my top and "
                                          "bottom borders, but they are "
                                          "hidden by default. Also notice "
                                          "that my text is really long, "
                                          "so it extends beyond a single "
                                          "line, and should wrap properly "
                                          "within the menu borders. This is "
                                          "a useful place to put "
                                          "instructions to the user about "
                                          "how to use the menu.",
                            epilogue_text="This is my epilogue. My borders "
                                          "are currently hidden.",
                            formatter=MenuFormatBuilder()
                            .set_title_align('center')
                            .set_subtitle_align('center')
                            .set_border_style_type(MenuBorderStyleType.
                                                   DOUBLE_LINE_BORDER)
